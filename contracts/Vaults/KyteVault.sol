 ...
 */
contract KyteVault is ERC20, Ownable, ReentrancyGuard {
    ...
    uint256 immutable  public depositFee;
    uint256 immutable  public withdrawFee;
    address immutable  public kyteFeeWallet;
    ...
    constructor (
        ...
        address _kyteFeeWallet

    ) public ERC20(
        _name,
        _symbol
    ) {
        require(_withdrawFee <= 500,"invalid withdraw fee");
        require(_depositFee <= 500,"invalid deposit fee");

        strategy = _strategy;
        depositFee = _depositFee;
        withdrawFee=_withdrawFee;
        kyteFeeWallet = _kyteFeeWallet;
    }
    ...
    function deposit(uint _amount) public nonReentrant {
        ...
        if(depositFee>0){
            uint256 _depositFee = _amount.mul(depositFee).div(MAX_FEE);
            want().safeTransfer(kyteFeeWallet, _depositFee);
    ...
    function withdraw(uint256 _shares) public {
        ...
        if(withdrawFee>0){
            uint256 _withdrawFee =  withdrawAmount.mul(withdrawFee).div(MAX_FEE);
            want().safeTransfer(kyteFeeWallet, _withdrawFee);
            withdrawAmount = withdrawAmount.sub(_withdrawFee);
        }
